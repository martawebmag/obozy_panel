<div class="flex flex-col min-h-screen bg-slate-100">
    ...
    <div class="bg-slate-400 border-b border-slate-300 shadow-sm">
        <div class="max-w-7xl mx-auto px-6 py-2 text-sm text-slate-700 flex items-center space-x-2">
            ...
            <span class="text-gray-600">››</span>
            <span class="font-medium text-sky-800">Formularz</span>
        </div>
    </div>
    ...
    <div class="flex-1 px-4 md:px-10 py-6 flex justify-center">
        <div class="flex flex-col md:flex-row bg-white rounded-2xl shadow-xl border border-slate-300 w-full max-w-4xl overflow-hidden">
                    ...
                    @foreach ($titles as $index => $title)
                        ...
                        <div
                            ...
                            class="flex flex-col items-center relative z-10 mb-6
                                {{ $isClickable ? 'cursor-pointer' : 'cursor-not-allowed pointer-events-none opacity-90' }}                           group">
                            <div
                                ...
                                group-hover:ring-2 ring-sky-500 font-medium text-xs transition-all duration-300">
                                {{ $thisStep }}
                            </div>

                            <div
                                class="mt-2 text-xs text-center
                                {{ $step == $thisStep ? 'text-sky-600 font-semibold' : 'text-slate-500 group-hover:text-sky-700' }}">
                                {{ $title }}
                            </div>
                        </div>
                    @endforeach
                    ...
                    @foreach($titles as $index => $title)
                        ...
                        <div
                            ...
                            <div class="w-8 h-8 flex items-center justify-center rounded-full
                                        {{ $step > $index ? 'bg-sky-700 text-white' : 'bg-slate-300 text-slate-500' }}
          ...
          <div class="flex-1 p-6 flex flex-col bg-white">
                ...
                <div class="mb-6">
                    <h2 class="text-md md:text-lg text-sky-800 text-center md:text-left font-semibold bg-slate-200 px-2">
                        {{ $titles[$step-1] }}
                    </h2>
                    <div class="h-0.5 bg-sky-800 w-full rounded-full"></div>
                </div>


                <div class="grid grid-cols-1 gap-x-12 gap-y-4 px-4">
                    @if($step==1)
                        <x-form.input-text name="imie" label="Imię" model="imie" required="true"/>
                        <x-form.input-text name="nazwisko" label="Nazwisko" model="nazwisko" required="true"/>
                        <x-form.input-text name="pesel" label="PESEL" model="pesel" required="true"/>
                        <x-form.input-text name="data_urodzenia" label="Data urodzenia" type="date" model="data_urodzenia" required="true"/>
                        <x-form.input-text name="email_dzielo" label="Email (dzielo.pl)" model="email_dzielo"
                        required="true"/>
                        <x-form.input-text name="email_prywatny" label="Email (prywatny)" model="email_prywatny" required="true"/>
                        <x-form.input-text name="telefon" label="Telefon" model="telefon" required="true"/>
                        <x-form.select name="plec" label="Płeć" :options="[''=>'--wybierz--','Kobieta'=>'Kobieta','Mężczyzna'=>'Mężczyzna']" model="plec" required="true"/>
                        <x-form.select name="wspolnota" label="Wspólnota akademicka" :options="[''=>'--wybierz--','Białystok'=>'Białystok','Bydgoszcz'=>'Bydgoszcz','Gdańsk'=>'Gdańsk','Katowice'=>'Katowice', 'Kielce'=>'Kielce','Kraków'=>'Kraków','Lublin'=>'Lublin','Łódź'=>'Łódź','Olsztyn'=>'Olsztyn','Opole'=>'Opole', 'Poznań'=>'Poznań','Rzeszów'=>'Rzeszów', 'Szczecin'=>'Szczecin','Toruń'=>'Toruń','Warszawa'=>'Warszawa','Wrocław'=>'Wrocław', 'Student zagraniczny'=>'Student zagraniczny']" model="wspolnota" required="true"/>
                    @elseif($step==2)
                        <x-form.input-text name="ulica" label="Ulica" model="ulica"/>
                        <x-form.input-text name="nr_domu" label="Nr domu" model="nr_domu" required="true"/>
                        <x-form.input-text name="nr_mieszkania" label="Nr mieszkania" model="nr_mieszkania"/>
                        <x-form.input-text name="kod_pocztowy" label="Kod pocztowy" model="kod_pocztowy" required="true"/>
                        <x-form.input-text name="poczta" label="Poczta" model="poczta" required="true"/>
                        <x-form.input-text name="miejscowosc" label="Miejscowość" model="miejscowosc" required="true"/>
                        <x-form.select name="diececja" label="Diecezja" model="diecezja" :options="[
                            ''=>'--wybierz--',
                    ...
                    @elseif($step==3)
                        <x-form.input-text name="imie_opiekuna" label="Imię" model="imie_opiekuna" required="true"/>
                        <x-form.input-text name="nazwisko_opiekuna" label="Nazwisko" model="nazwisko_opiekuna" required="true"/>
                        <x-form.input-text name="telefon_opiekuna" label="Telefon" model="telefon_opiekuna" required="true"/>
                    @elseif($step==4)
                        <x-form.text-area name="zdrowie" label="Zdrowie" model="zdrowie" class="md:col-span-2"/>
                        <x-form.select name="dieta" label="Dieta" :options="[''=>'--wybierz--', 'Tak'=>'Tak','Nie'=>'Nie']" model="dieta"/>
                        @if($dieta=='Tak')
                        <x-form.input-text name="jaka_dieta" label="Jaka dieta?" model="jaka_dieta" required="true"/>
                        <x-form.text-area name="dieta_info" label="Informacje o diecie (alergie, nietolerancje itp.)" model="dieta_info" class="md:col-span-2"/>
                        @endif

                    @elseif($step==5)
                        <x-form.select name="obrona" label="Czy przystępujesz w tym roku do obrony pracy licencjackiej / magisterskiej /
                            inżynierskiej i termin obrony pokrywa się z czasem obozu?" :options="[''=>'--wybierz--', 'Tak'=>'Tak','Nie'=>'Nie']" model="obrona" />
                        <x-form.select name="sesja" label="Czy przewidywany termin sesji egzaminacyjnej na twojej uczelni pokrywa się z
                            czasem obozu?" :options="[''=>'--wybierz--', 'Tak'=>'Tak','Nie'=>'Nie']" model="sesja"/>
                        @if($sesja=='Tak')
                            <x-form.input-text name="koniecSesji" label="Kiedy kończysz sesję?" type="date" model="koniecSesji"/>
                        @endif
                        ...
                        <x-form.select name="chor" label="Czy chcesz zaangażować się w chór obozowy?" :options="[''=>'--wybierz--', 'Tak'=>'Tak','Nie'=>'Nie']" model="chor"/>
                        <x-form.input-text name="instrument" label="Gram na instrumencie:" model="instrument"/>
                        <x-form.select name="posluga" label="Czy chciałbyś zaangażować się w posługę liturgiczną podczas obozu?" :options="[''=>'--wybierz--', 'Tak'=>'Tak','Nie'=>'Nie']" model="posluga"/>
                        <x-form.select name="medycyna" label="Czy jesteś studentem kierunku medycznego?" :options="[''=>'--wybierz--', 'Tak'=>'Tak','Nie'=>'Nie']" model="medycyna"/>
                        <x-form.text-area name="uwagi" label="Uwagi" model="uwagi" class="md:col-span-2"/>

                    @elseif($step==6)
                        <x-form.checkbox name="regulamin" label="Zapoznałem się z regulaminem obozu i akceptuję jego wymagania => Regulamin obozu" model="regulamin" required="true"/>
                        <x-form.checkbox name="rodo" label="Wyrażam zgodę na przetwarzanie moich danych osobowych przesłanych przy pomocy formularza dla potrzeb niezbędnych do uczestnictwa w obozach integracyjno-formacyjnych organizowanych przez Fundację 'Dzieło Nowego Tysiąclecia' zgodnie z dokumentem elektronicznym: KLAUZULA DO 'ZGŁOSZENIA NA OBÓZ FUNDACJI DZIEŁO NOWEGO TYSIĄCLECIA'" model="rodo" required="true"/>
                        <x-form.checkbox name="wizerunek" label="Wyrażam zgodę na przetwarzanie mojego wizerunku do celów związanych z promocją Fundacji 'Dzieło Nowego Tysiąclecia', co oznacza, że fotografie, filmy lub nagrania wykonane podczas obozu mogą być zamieszczone na stronie internetowej i mediach społeczościowych Fundacji lub wykorzystywane w jej materiałach promocyjnych" model="wizerunek" required="true"/>
                        <x-form.checkbox name="ochrona_maloletnich" label="Zapoznałem się z Polityką Ochrony Małoletnich Fundacji „Dzieło Nowego Tysiąclecia” - https://dzielo.pl/polityka-ochrony-maloletnich" model="ochrona_maloletnich" required="true"/>
                    @endif

...
<div class="min-h-screen bg-slate-100 py-10 px-6">
    <div class="max-w-6xl mx-auto bg-white shadow-md rounded-xl p-8 space-y-10 border border-slate-200">
        ...
        <h1 class="text-2xl font-bold text-slate-800 border-b pb-3">📝 Edycja danych stypendysty</h1>
    ...
    <form action="{{ route('biuro.stypendysci.update', $stypendysta->id ?? '') }}" method="POST" class="space-y-10">
            @csrf
            @method('PUT')
            ...
            <section>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <x-form.input-text name="data_urodzenia" type="date" label="Data urodzenia" :value="$stypendysta->data_urodzenia" />
            ...
            <!-- 🔹 RODZICE / OPIEKUN -->
            <section>
                <h2 class="text-xl font-semibold text-slate-700 mb-4 border-b pb-1">Rodzice / Opiekun prawny</h2>
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <x-form.input-text name="nazwisko_rodowe_matki" label="Nazwisko rodowe matki" :value="$stypendysta->nazwisko_rodowe_matki" />
                    ...
                    <x-form.input-text name="nazwisko_opiekuna_prawnego" label="Nazwisko opiekuna" :value="$stypendysta->nazwisko_opiekuna_prawnego" />
                    <x-form.input-text name="telefon_opiekuna_prawnego" label="Telefon opiekuna" :value="$stypendysta->telefon_opiekuna_prawnego" />
                    <x-form.input-text name="pesel_opiekuna_prawnego" label="PESEL opiekuna" :value="$stypendysta->pesel_opiekuna_prawnego" />
                    <x-form.input-text name="email_opiekuna_prawnego" label="Email opiekuna" :value="$stypendysta->email_opiekuna_prawnego" />
                    <x-form.input-text name="prawa_rodzicielskie" label="Prawa rodzicielskie" :value="$stypendysta->prawa_rodzicielskie" />
            ...
            <section>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <x-form.text-area name="dietaInfo" label="Dodatkowe informacje o diecie" :value="$stypendysta->dietaInfo" />
                    <x-form.select name="tshirt" label="Rozmiar T-shirtu" :options="['XS'=>'XS','S'=>'S','M'=>'M','L'=>'L','XL'=>'XL']" :value="$stypendysta->tshirt" />
                    ...
                    <x-form.input-text name="data_przyjazdu" type="date" label="Data przyjazdu" :value="$stypendysta->data_przyjazdu" />
                    <x-form.input-text name="data_wyjazdu" type="date" label="Data wyjazdu" :value="$stypendysta->data_wyjazdu" />
                </div>
            </section>
            ...
            <section>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <x-form.checkbox name="informacje" label="Zgoda na przekazywanie informacji" :checked="$stypendysta->informacje" />
                    <x-form.checkbox name="elektronika" label="Zgoda na korzystanie z elektroniki" :checked="$stypendysta->elektronika" />
                    <x-form.checkbox name="rodo" label="Zgoda RODO" :checked="$stypendysta->rodo" />
                    <x-form.checkbox name="wizerunek" label="Zgoda na publikację wizerunku" :checked="$stypendysta->wizerunek" />
                    <x-form.checkbox name="ochrona_maloletnich" label="Zgoda – ochrona małoletnich" :checked="$stypendysta->ochrona_maloletnich" />
                </div>
            </section>

            <!-- 🔹 PRZYCISK -->
            <div class="pt-6 border-t">
                <button type="submit" class="bg-orange-600 hover:bg-orange-700 text-white font-semibold px-6 py-2 rounded-lg shadow">
                    💾 Zapisz zmiany
                </button>
            </div>
        </form>

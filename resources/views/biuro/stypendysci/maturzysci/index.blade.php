...
<div class="mb-2 text-sm text-slate-600">
    Znaleziono  <span class="font-bold">{{ $maturzysci->total() }}</span> uczestników.
</div>
...
<div class="overflow-x-auto bg-white rounded-lg shadow border border-slate-200">
    <table class="min-w-full text-sm text-slate-700">
        ...
        <tbody class="divide-y divide-slate-200 text-xs">
            @forelse($maturzysci as $maturzysta)
                <tr class="odd:bg-white even:bg-slate-50 hover:bg-slate-100 transition">
                    <td class="px-2 py-1 truncate"> {{ ($maturzysci->currentPage() - 1) * $maturzysci->perPage() + $loop->iteration }}</td>
                    <td class="px-2 py-1 truncate">{{ $maturzysta->typ_uczestnika }}</td>
                    <td class="px-2 py-1 truncate">{{ $maturzysta->imie }}</td>
                    <td class="px-2 py-1 truncate">{{ $maturzysta->nazwisko }}</td>
                    <td class="px-2 py-1 truncate">{{ $maturzysta->pesel }}</td>
                    <td class="px-2 py-1 truncate">{{ $maturzysta->diecezja }}</td>
                    <td class="px-2 py-1 truncate">{{ $maturzysta->ulica }}</td>
                    <td class="px-2 py-1 truncate">{{ $maturzysta->miejscowosc }}</td>

                    <td class="px-2 py-1 flex space-x-1 text-xs">
                        <a href="{{ route('biuro.stypendysci.edit', $maturzysta->id) }}"
                        ...
                        <button type="button" onclick="openDeleteModal({{ $maturzysta->id }})"
    ...
    <div class="mt-4 flex justify-center space-x-1 text-sm">
        ...
        <!-- Następna strona -->
        @if ($maturzysci->hasMorePages())
            <a href="{{ $maturzysci->nextPageUrl() }}"
            class="px-3 py-1 text-slate-700 border border-slate-200 rounded-md hover:bg-slate-100">&gt;</a>
        @else
            <span class="px-3 py-1 text-slate-400 border border-slate-200 rounded-md cursor-not-allowed">&gt;</span>
        @endif
